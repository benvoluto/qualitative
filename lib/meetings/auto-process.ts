 ...
 * Runs in the background after a meeting is synced.
 */
...
export async function autoProcessAndExtract(
  ...
  try {
    ...
    const existingExtracts = await extracts.getExtractsByMeetingId(accountId, meetingId);
    ...
    const extractResult = await processMeetingExtracts(accountId, meetingId);
    ...
    const freshMeeting = await meetings.getMeetingById(accountId, meetingId);

    if (!freshMeeting || meetingExtracts.length === 0) return result;

    const customer = freshMeeting.customer_id
      ? await customers.getCustomerById(accountId, freshMeeting.customer_id)
      : null;
    ...
    try {
      ...
      await meetings.updateMeeting(accountId, meetingId, { user_notes: notesSummary });
...
export async function autoProcessMeetings(
  ...
  for (let i = 0; i < meetingIds.length; i += concurrency) {
    ...
    const batchResults = await Promise.all(
      batch.map((id) => autoProcessAndExtract(accountId, id, userId))
    );
