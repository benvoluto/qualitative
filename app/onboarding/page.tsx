...
import { accounts, users } from "@/lib/db";
import {
...
export default async function OnboardingPage({
  ...
  // Prefer the user's display name as the workspace default. If the account name
  // was auto-set to the email domain (the pre-fix default), upgrade it. Anything
  // the user has manually entered wins.
  const user = await users.getUserById(ctx.userId);
  const accountNameLooksAutoGenerated =
    account.name === defaultDomain || account.name === "Workspace";
  const workspaceDefaultName =
    accountNameLooksAutoGenerated && user?.name?.trim()
      ? user.name.trim()
      : account.name;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {step === "workspace" && (
          <WorkspaceStep
            initialName={workspaceDefaultName}
